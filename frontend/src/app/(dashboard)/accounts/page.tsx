...
function AccountForm({
  ...
  return (
    <Card className="animate-fade-in">
      <CardHeader>
        <CardTitle className="text-base">{isEditing ? 'Edit Account' : 'New Account'}</CardTitle>
      </CardHeader>
      <CardContent>
...
function AccountsContent() {
  ...
  return (
    <div>
      <Header title="Accounts" subtitle="Manage bank, cash, UPI, and card accounts for payments" />
        ...
        >
            ...
            {accounts.map((account) => (
              <Card key={account.id}>
                <CardContent className="p-6">
                  ...
                  {account.bankName && (
                    <p className="text-sm text-muted-foreground mt-1">{account.bankName}</p>
                  )}
                  {account.lastFour && (
                    <p className="text-xs text-muted-foreground mt-1">•••• {account.lastFour}</p>
                  )}
          ...
          {!accounts.length && (
            <div className="p-8 text-center text-muted-foreground">
              No accounts yet. Add Cash, Bank, or UPI accounts to track where payments go.
            </div>
          )}
        </QueryState>
      </div>
